...
export const PaymentProofView: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await paymentService.submitProof({
      ...
      receiptUrl: receiptImage || 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=400',
      notes: notes.trim() || undefined
    });
    setLoading(false);

    if (res.success) {
      success('Payment proof submitted successfully! Pending verification by Central Operations.');
      setTransactionId('');
      refreshUser();
    } else {
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          {latestProof && (
            <p className="text-xs text-slate-500 mt-1">
              Latest Submission: <strong className="font-mono">{latestProof.transactionId}</strong> ({latestProof.status})
            </p>
          )}
      ...
      <div className="p-6 sm:p-8 bg-slate-900 text-white rounded-3xl space-y-6 shadow-md border border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-5 bg-slate-800/90 rounded-2xl border border-slate-700 space-y-3 relative group">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <h3 className="text-base font-extrabold text-white">JS Bank</h3>
                </div>
              </div>
            ...
            <div className="space-y-2 pt-1 border-t border-slate-700/60 text-xs">
              ...
              <div className="flex justify-between items-center px-1">
                ...
                <span className="font-bold text-white text-sm">Khizer Abbas</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 bg-slate-800/90 rounded-2xl border border-slate-700 space-y-3 relative group">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <h3 className="text-base font-extrabold text-white">NayaPay</h3>
                </div>
              </div>
            ...
            <div className="space-y-2 pt-1 border-t border-slate-700/60 text-xs">
              <div className="flex justify-between items-center bg-slate-900/60 p-2.5 rounded-xl border border-slate-800">
                <div>
                  <span className="text-[10px] text-slate-400 block font-medium">NayaPay Number</span>
                  <span className="font-mono text-base font-black text-emerald-400 tracking-wider">
                    03246362855
                  </span>
                </div>
        ...
        {/* Verification Notice & WhatsApp Action Button */}
        <div className="p-4 sm:p-5 rounded-2xl bg-emerald-950/40 border border-emerald-500/30 space-y-4">
          <div className="flex items-start gap-3 text-xs text-slate-300">
            ...
            <p className="leading-relaxed">
              <strong className="text-white">Manual Verification Notice:</strong> Payment is manually verified by central administration after receipt of transfer proof. Payments are <span className="text-amber-300 font-semibold">NOT automatically verified</span>. Please submit your proof below and send it directly on WhatsApp.
            </p>
          </div>

          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2 border-t border-emerald-500/20">
            <span className="text-xs text-emerald-300 font-medium">
              Official WhatsApp Desk: <strong className="font-mono text-white">03246362855</strong>
            </span>
      ...
      {/* Submission Form (Shown if not yet active or if rejected) */}
      {profile.accountStatus !== 'active' && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="p-4 border-2 border-dashed border-slate-200 rounded-2xl text-center space-y-2">
              ...
              />
              {receiptImage && <span className="text-[11px] text-emerald-600 font-bold block">Screenshot Attached ✓</span>}
            </div>
            ...
            >
              ...
              <span>{loading ? 'Submitting...' : 'Submit Payment Proof'}</span>
            </button>
          </form>
        </div>
      )}
